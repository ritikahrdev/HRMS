...
function scoreApplicant(app, job) {
  ...
  const skillScore = req.length ? req.filter((s) => have.some((h) => h.includes(s) || s.includes(h))).length / req.length : 1;
...
router.post('/jobs/:id/applicants', P, upload.single('resume'), async (req, res) => {
  try {
    ...
    const app = { name: b.name, email: b.email || '', phone: b.phone || '', experience_years: Number(b.experience_years) || 0, skills: b.skills || '', source: b.source || 'Manual' };
...
router.put('/applicants/:id', P, async (req, res) => {
  try {
    ...
    const b = req.body || {};
    await db.prepare('UPDATE applicants SET stage=?, notes=? WHERE id=?').run(b.stage || a.stage, b.notes != null ? b.notes : a.notes, a.id);
    res.json({ ok: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.post('/jobs/:id/auto-shortlist', P, async (req, res) => {
  try {
    ...
    for (const a of applicants) {
      ...
      await db.prepare('UPDATE applicants SET score = ?, stage = ? WHERE id = ?').run(score, stage, a.id);
...
router.post('/applicants/:id/hire', P, async (req, res) => {
  try {
    ...
    for (let i = 0; i < ONBOARDING.length; i++) {
      await db.prepare('INSERT INTO onboarding_tasks (employee_id, title, position) VALUES (?, ?, ?)').run(employee.id, ONBOARDING[i], i + 1);
    }
