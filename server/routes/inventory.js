...
router.get('/stats', requirePerm('settings:manage'), async (req, res) => {
  try {
    const stats = await db.prepare(`
      ...
      FROM inventory GROUP BY category ORDER BY category`).all();
...
router.post('/', requirePerm('settings:manage'), async (req, res) => {
  try {
    ...
    const r = await db.prepare(
      `INSERT INTO inventory (name, category, quantity, available, condition, serial_number, purchase_date, purchase_price, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
...
router.put('/:id', requirePerm('settings:manage'), async (req, res) => {
  try {
    ...
    const assigned_to = req.body.assigned_to !== undefined ? (req.body.assigned_to || null) : item.assigned_to;
    ...
    await db.prepare(
      `UPDATE inventory SET name=?, category=?, quantity=?, available=?, assigned_to=?, condition=?, serial_number=?, purchase_date=?, purchase_price=?, notes=?, updated_at=datetime('now') WHERE id=?`
