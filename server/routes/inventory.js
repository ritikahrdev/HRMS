...
router.get('/', requirePerm('settings:manage'), (req, res) => {
  ...
  const rows = category && VALID_CATEGORIES.includes(category)
    ? db.prepare(`SELECT i.*, e.name AS assigned_name FROM inventory i LEFT JOIN employees e ON e.id = i.assigned_to WHERE i.category = ? ORDER BY i.category, i.name`).all(category)
    : db.prepare(`SELECT i.*, e.name AS assigned_name FROM inventory i LEFT JOIN employees e ON e.id = i.assigned_to ORDER BY i.category, i.name`).all();
  res.json({ items: rows });
});

// GET summary stats (admin only)
router.get('/stats', requirePerm('settings:manage'), (req, res) => {
