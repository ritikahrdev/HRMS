...
router.get('/mine', requireLogin, async (req, res) => {
  try {
    ...
    res.json({ goals: await db.prepare('SELECT * FROM goals WHERE employee_id = ? ORDER BY created_at DESC').all(empId) });
  } catch (e) {
...
router.get('/:employeeId', requireLogin, async (req, res) => {
  try {
    ...
    res.json({ goals: await db.prepare('SELECT * FROM goals WHERE employee_id = ? ORDER BY created_at DESC').all(req.params.employeeId) });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
...
router.post('/', requireLogin, async (req, res) => {
  try {
    ...
    if (target_date) {
      const d = /^\d{4}-\d{2}-\d{2}$/.test(target_date) ? new Date(target_date + 'T00:00:00Z') : null;
      if (!d || isNaN(d.getTime()) || d.toISOString().slice(0, 10) !== target_date) {
        return res.status(400).json({ error: 'Target date must be a valid date (YYYY-MM-DD).' });
      }
      if (target_date < new Date().toISOString().slice(0, 10)) {
