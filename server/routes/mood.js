...
router.get('/dashboard', requirePerm('reports:view'), async (req, res) => {
  try {
    ...
    const totalActive = (await db.prepare("SELECT COUNT(*) AS n FROM employees WHERE status='active'").get()).n;
    ...
    const perEmployee = await db.prepare(`
