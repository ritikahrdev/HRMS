...
async function notifyUsers(userIds, { type, title, body, link }) {
  ...
  for (const id of userIds) await ins.run(id, type || null, title, body || null, link || null);
}

// Notifies everyone with a login except the actor.
async function notifyEveryone(exceptUserId, payload) {
  const ids = (await db.prepare('SELECT id FROM users').all()).map((u) => u.id).filter((id) => id !== exceptUserId);
