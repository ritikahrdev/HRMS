...
async function applyLeaveDecision(id, decision, comment, approverUserId) {
  ...
  await db
    .prepare(
      "UPDATE leave_requests SET status = ?, comment = ?, approver_id = ?, decided_at = datetime('now') WHERE id = ?"
    )
  ...
  if (decision === 'approved') {
    ...
    for (let ds = lr.from_date; ds <= lr.to_date; ds = addDays(ds, 1)) {
      const ex = await db
        .prepare('SELECT id, check_in FROM attendance WHERE employee_id = ? AND date = ?')
        .get(lr.employee_id, ds);
      if (!ex) {
        await db
          .prepare('INSERT INTO attendance (employee_id, date, status) VALUES (?, ?, ?)')
          .run(lr.employee_id, ds, markStatus);
      } else if (!ex.check_in) {
        await db
          .prepare('UPDATE attendance SET status = ? WHERE id = ?')
          .run(markStatus, ex.id);
      }
    }
  }

  const emp = await db
    .prepare('SELECT name, email FROM employees WHERE id = ?')
    .get(lr.employee_id);
  if (emp && emp.email) {
    await sendMail({
      to: emp.email,
...
async function applyReimbursementDecision(id, decision, comment, approverUserId) {
  ...
  await db
    .prepare(
      "UPDATE reimbursements SET status = ?, comment = ?, approver_id = ?, decided_at = datetime('now') WHERE id = ?"
    )
